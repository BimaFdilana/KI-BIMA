    ...
    <div class="container-fluid px-4 py-6">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="flex items-center space-x-4 mb-6">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Edit Pos Infaq</h1>
                    <p class="text-gray-600">Perbarui informasi pos penggalangan dana: <span
                            class="font-semibold text-red-600">{{ $infaqList->name }}</span></p>
                </div>
            </div>

            <!-- Form Card -->
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                <form action="{{ route('infaq.list.update', $infaqList->id) }}" method="POST" class="p-6 space-y-6">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="relative">
                                ...
                                <select name="category" id="category"
                                    ...
                                    @foreach ($categories as $value => $label)
                                        <option value="{{ $value }}"
                                            {{ old('category', $infaqList->category) == $value ? 'selected' : '' }}>
                                            {{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="dana_dibutuhkan" id="dana_dibutuhkan"
                                    value="{{ old('dana_dibutuhkan', $infaqList->dana_dibutuhkan) }}"
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="description" id="description" rows="4"
                            ...
                            placeholder="Jelaskan tujuan penggalangan dana ini...">{{ old('description', $infaqList->description) }}</textarea>
                        @error('description')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center space-x-3 p-4 bg-gray-50 rounded-lg border border-gray-200">
                        <div class="flex items-center h-5">
                            <input id="is_active" name="is_active" type="checkbox" value="1"
                                {{ old('is_active', $infaqList->is_active) ? 'checked' : '' }}
