...
class CategoryDataTable extends DataTable
{
    ...
    private const TABLE_ID = 'category-table';
    ...
    public function dataTable(EloquentBuilder $query): EloquentDataTable
    {
        return datatables()
            ...
            ->addColumn('data', function ($row) {
                ...
                if (!$imageUrl) {
                    ...
                    <div class="ml-3">
                        ...
                        <p class="text-xs text-gray-500">#' . ($row->subcategories->count() ?? 'N/A') . ' Subcategory</p>
                    </div>
                </div>';
                }
            ...
            ->addColumn('action', function ($row) {
                ...
                $button .= '<button data-table="' . self::TABLE_ID . '" data-id="' . $row->id . '" class="edit-category-modal rounded bg-yellow-100 text-yellow-600 hover:text-yellow-600 hover:bg-yellow-200 text-sm font-medium px-2 py-1.5 cursor-pointer hover:scale-125 transition-all inline-flex space-x-1 items-center">';
     ...
     */
    public function query(Category $model): EloquentBuilder
    {
        return $model->newQuery()
            ...
            ->selectRaw('
                (SELECT COUNT(*) FROM sub_categories WHERE sub_categories.category_id = categories.id) as subcategory_count
            ')
            ->selectRaw('
                (
                    SELECT COUNT(*)
                    FROM barang
                    WHERE barang.subcategory_id IN (
